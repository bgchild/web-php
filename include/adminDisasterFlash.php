 ...
 * 主页flash管理
 */
class adminDisasterFlash extends adminbase
{
    ...
    public function addImg()
    {
        ...
        if ($rid) {
            ...
            $result = $this->db->edit('form__bjzh_flash', $datas, "id=$rid");
            return true;
        } else {
            if (!$thumb_url) return false;
            $datas['img'] = $thumb_url;
            $result = $this->db->add('form__bjzh_flash', $datas);
    ...
    public function getOneInfo()
    {
        ...
        $where = "id=$rid";
        $one = $this->db->get_one('form__bjzh_flash', $where);
    ...
    public function getflash($limit)
    {
        ...
        $flashinfo = $this->db->getall('form__bjzh_flash', $where, array(limit => $limit), $fields = '*', ' order by orderlist ASC');
        return $flashinfo;
    }
        ...
        foreach ($list as $key => $val) {
            foreach ($pid as $k => $v) {
                if ($k == $key) {
                    $datas['orderlist'] = $val;
                    $where = "id=$v";
                    $afferd = $this->db->edit('form__bjzh_flash', $datas, $where);
                }
            }
        }
        return true;
