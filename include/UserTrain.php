...
class UserTrain extends user {
     ...
     * 获取我的培训记录
     */
    public function getMyRecords($query, $limit, $arg) {
        $q1 = "trainerid=$arg[0]";
        foreach ( $query as $key => $val ) {
            $val = trim ( $val );
            if ($key == 'startdate') {
                $val = strtotime($val);
                $q1 .= " and startdate >=$val ";continue;
            }
            if ($key == 'enddate') {
                $val = strtotime($val);
                $q1 .= " and enddate <=$val ";continue;
            }
            $q1 .= " and $key='$val' ";
        }

        $this->count = $this->db->get_count ( 'form__bjhh_captainmanage_addtrain', $q1 );
        return $this->db->getall('form__bjhh_captainmanage_addtrain',$q1,array ('limit' => $limit ),'*'," order by recordID DESC ");
    }
    ...
    public function init($id){
        ...
        return $this->db->get_one('form__bjhh_captainmanage_addtrain',$q1);
    }
    ...
    public function editRecord($post){
        ...
        $datas=array();
        $datas['trainer'] = $post['trainer'];
        $datas['startdate'] = strtotime($post['startdate']);
        $datas['enddate'] = strtotime($post['enddate']);
        $datas['trainaddress'] = $post['trainaddress'];
        $datas['traincontent'] = $post['traincontent'];

        if($post['rid']) {
            $affected = $this->db->edit('form__bjhh_captainmanage_addtrain', $datas, " recordid=".$post['rid']) ;
            return true;
        } else {
            return false;
        }
    ...
    public function addRecord($post, $ids){
        foreach($ids as $k=>$v){
            $datas[$k]['startdate'] = strtotime($post['startdate']);
            $datas[$k]['enddate'] = strtotime($post['enddate']);
            $datas[$k]['trainaddress'] = $post['trainaddress'];
            $datas[$k]['traincontent'] = $post['traincontent'];
            $person = $this->db->get_one('form__bjhh_volunteermanage_volunteerbasicinfo', "recordid=$v") ;
            $datas[$k]['trainer'] = $person['name'];
            $datas[$k]['trainerid'] = $v;
            $datas[$k]['sign'] = $_SESSION['sign'];
        }

        foreach($datas as $k=>$v){
            $affected=$this->db->add('form__bjhh_captainmanage_addtrain', $v) ;
