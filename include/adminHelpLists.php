<?php
include_once('adminBase.php');

/**
 *
 * 灾害-求助模块
 */
class adminHelpLists extends adminbase
{
    public function __construct()
    {
        parent::__construct();
    }

    //初始化求助列表
    public function init($query, $limit,$isSupper)
    {
        $q1 = " a.volunteerid=b.recordid ";
        if (($query && !$query['secity']) || !$query) {
            $q1 .= " and a.sign='$_SESSION[sign]'";
        }

        foreach ($query as $key => $val) {
            $val = trim($val);
            if ($key == 'dname') {
                $q1 .= " and a.name like '%$val%' ";
                continue;
            }

            if ($key == 'address') {
                $q1 .= " and a.address like '%$val%' ";
                continue;
            }

            if ($key == 'place') {
                $q1 .= " and a.place like '%$val%' ";
                continue;
            }

            if ($key == 'description') {
                $q1 .= " and a.description like '%$val%' ";
                continue;
            }

            if ($key == 'sdate') {
                $val = strtotime($val);
                $q1 .= " and a.addtime >=$val ";
                continue;
            }
            if ($key == 'edate') {
                $val = strtotime($val);
                $q1 .= " and a.addtime <=$val ";
                continue;
            }

            if ($key == 'status') {
                $q1 .= " and a.status = $val ";
                continue;
            }

            if ($key == 'secity' && $val != 'all') {
                $q1 .= " and a.sign = '$val' ";
            }

            if ($key == 'secity' && $val == 'all' && !$isSupper) {
                $q1 .= " and a.sign IN ('$query[signs]') ";
            }
        }

        //查询字段，vname上报志愿者名称
        $fields = "a.*,b.name as vname,b.mobile";

        $this->count = $this->db->get_relations_count('form__bjzh_help', 'form__bjzh_subscribe', $q1);
        return $this->db->get_relations_info('form__bjzh_help', 'form__bjzh_subscribe', $q1, array('limit' => $limit), " order by a.recordid DESC ", $fields);

    }

    //获取求助详情
    public function getAllLists($query,$isSupper)
    {
        $q1 = " a.volunteerid=b.recordid ";
        if (($query && !$query['secity']) || !$query) {
            $q1 .= " and a.sign='$_SESSION[sign]'";
        }

        foreach ($query as $key => $val) {
            $val = trim($val);
            if ($key == 'dname') {
                $q1 .= " and a.name like '%$val%' ";
                continue;
            }

            if ($key == 'sdate') {
                $val = strtotime($val);
                $q1 .= " and a.addtime >=$val ";
                continue;
            }
            if ($key == 'edate') {
                $val = strtotime($val);
                $q1 .= " and a.addtime <=$val ";
                continue;
            }

            if ($key == 'status') {
                $q1 .= " and a.status = $val ";
                continue;
            }

            if ($key == 'secity' && $val != 'all') {
                $q1 .= " and a.sign = '$val' ";
            }

            if ($key == 'secity' && $val == 'all' && !$isSupper) {
                $q1 .= " and a.sign IN ('$query[signs]') ";
            }
        }

        //查询字段，vname上报志愿者名称
        $fields = "a.*,b.name as vname,b.mobile";
        return $this->db->get_relations_info('form__bjzh_help', 'form__bjzh_subscribe', $q1, array(limit => '0,9999999'), " order by a.recordid DESC ", $fields);
    }

    //获取求助详情
    public function getInfo($rid)
    {
        $where = " a.volunteerid=b.recordid and a.recordid = $rid";
        $fields = "a.*,b.name as vname,b.mobile";
        return $this->db->get_relations_one('form__bjzh_help', 'form__bjzh_subscribe', $where, $fields);
    }

    //更新求助信息
    public function update($data, $rid)
    {
        return $this->db->edit('form__bjzh_help', $data, array('recordid' => $rid), 'recordid');
    }

    //删除求助信息
    public function delete($rid)
    {
        return $this->db->drop('form__bjzh_help', array('recordid' => $rid), 'recordid');
    }

    public function getCount()
    {
        return $this->count;
    }

    /**
     * 创建格式化数组
     */
    public function formatData($list)
    {
        $array = [];
        foreach ($list as $k => $v) {
            $array[$k]['name'] = $v['name'] ? $v['name'] : '不详';
            $array[$k]['address'] = $v['address'] ? $v['address'] : '不详';
            $array[$k]['place'] = $v['place'] ? $v['place'] : '不详';
            $array[$k]['description'] = $v['description'] ? $v['description'] : '不详';
            $array[$k]['vname'] = $v['vname'] ? $v['vname'] : '不详';
            $array[$k]['mobile'] = $v['mobile'] ? $v['mobile'] : '不详';
            $array[$k]['addtime'] =  $v['addtime'] ? date("Y-m-d", $v['addtime']) : '不详';
        }

        return $array;
    }

    /**
     * excel头部标题
     */
    function h_array()
    {
        $h_array = array(
            '求助需求', '求助地点', '事发地点', '求助描述', '求助人', '联系方式', '上报时间'
        );
        return $h_array;
    }

    /**
     * 导出
     */
    function batch_exp($exp_cfg, $expdata)
    {
        require_once('PHPExcel.php');
        $objPHPExcel = new PHPExcel();
        // Set properties
        $objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
            ->setLastModifiedBy("Maarten Balliauw")
            ->setTitle("Office 2007 XLSX Test Document")
            ->setSubject("Office 2007 XLSX Test Document")
            ->setDescription("Test document for Office 2007 XLSX, generated using PHP classes.")
            ->setKeywords("office 2007 openxml php")
            ->setCategory("Test result file");

        //设置表头
        $key = ord("A");
        foreach ($exp_cfg as $v) {
            $column = chr($key);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($column . '1', $v);
            $objPHPExcel->getActiveSheet()->getColumnDimension($column)->setWidth(20);
            $key += 1;
        }

        $k = 2;
        foreach ($expdata as $row) {
            $span = ord("A");
            foreach ($row as $keyName => $value) {
                $j = chr($span);
                if (is_array($value)) {
                    foreach ($value as $v) {
                        $j = chr($span);
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($j . $k, $v);

                        $span++;
                    }
                } else {
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($j . $k, $value);
                }
                $span++;
            }
            $k++;
        }


        $title = "求助列表";
        $name = $title . '.xls';
        if (preg_match('/MSIE/', $_SERVER['HTTP_USER_AGENT'])) {
            $name = rawurlencode($name);
        }
        // Rename sheet
        $objPHPExcel->getActiveSheet()->setTitle($title);

        // Redirect output to a client’s web browser (Excel5)
        $objPHPExcel->setActiveSheetIndex(0);
        ob_end_clean(); // 输出xls文档之前清理下缓存,以防部分机器上出现乱码
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="' . $name . '"');
        header('Cache-Control: max-age=0');
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
        $objWriter->save('php://output');
        exit;
    }
}

?>