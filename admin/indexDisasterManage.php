...
switch ($act) {
    case 'init' :
        $list = $admin_manage->initDisaster();
        include get_admin_tpl('indexDisasterManage');
        break;
    ...
    case 'edit' :
        ...
        include get_admin_tpl('indexDisasterManageEdit');
        break;
    ...
    case 'save' :
        $rid = trim($_POST['rid']);
        $name = $_POST['name'];
        $backurl = $admin_op->getBackUrl('indexDisasterManage.php');
        if ($admin_manage->editTitle($rid, $name)) {
            $db->get_show_msg("indexDisasterManage.php", '保存成功');
        } else {
