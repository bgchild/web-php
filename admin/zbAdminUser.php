...
if ($_GET['spm']) {
    ...
    if (!$spm)
        header("Location: zbAdminUserList.php");
...
if (isset($_POST['doedit'])) {
    ...
    if ($db->edit('form__bjhh_admin', $userinfo, "recordid=$rid")) {
        ...
        if ($arg[1] == $get_one['u_name']) {
            ...
            $db->get_show_msg('zbAdminUserList.php', '修改成功');
        }
...
if (isset($_POST['dosubmit'])) {
    ...
    if ($db->add('form__bjhh_admin', $userinfo)) {
        ...
        $admin_op->doLog($arr);
        $db->get_show_msg('zbAdminUserList.php', '添加成功');
    } else {
        $db->get_show_msg($url, '添加失败');
    }
//print_r($userinfo);
}
include get_admin_tpl('zb_admin_user');
