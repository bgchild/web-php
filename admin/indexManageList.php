...
switch ($act) {
	case 'init' :
		...
		if(!$rid){$db->get_show_msg('indexManage.php','参数错误！');}
		$page = _get_page ( 10 ); 
		...
		include get_admin_tpl('indexManageList');
		break;
	case 'edit' :
		$rid = get_url($_GET['rid']);
		if(!$rid){$db->get_show_msg('indexManageList.php','参数错误！');}
		$detail =$admin_manage->zTitle($rid);
		$column = $admin_manage->fTitle($detail['fid']);
		$backurl=$admin_op->getBackUrl('indexManageList.php');
	...
	case 'delete' :
		...
		header("Location:indexManageList.php?rid=".$cid);
		break;
	case 'add' :
		$post=array();
	...
	case 'save' :
		...
		if($admin_manage->editNews($post)){
			$db->get_show_msg("indexManageList.php?rid=$cid", '保存成功');
		}else{
			$db->get_show_msg($backurl, '保存失败');
		}
