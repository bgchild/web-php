...
if ($obj->orderInfo()) {
			$db->get_show_msg ( 'adminflash.php', '排序成功' );
           } 
}

if($_GET['recordid']) {
	$rid=get_url($_GET['recordid']);
	if(!$rid) $db->get_show_msg($obj->getBackurl("adminflash.php"),"参数错误！！");
	if($obj->deleteOneRecord($rid)>0) 
		$db->get_show_msg("adminflash.php", "删除成功！");
	else 
		$db->get_show_msg("adminflash.php","删除失败！请再次尝试！");
}

include get_admin_tpl ( 'admin_flash' );
?>
