...
include_once(INCLUDE_PATH.'servicetimeCount.php');
$admin_op=new servicetimeCount();
...
$searchItems=array('ctype','time_start','time_stop','secity','lower','team'); 
if(isset($_GET['doSearch'])){
	foreach ($searchItems as $v){
		...
		header("Location:servicetimeCount.php?info=".set_url($infos));
	}
}
...
// 工时统计,不是统计每页的,但也需要分页
if(empty($infos['ctype']) || $infos['ctype']==1) {
	$totalNum = 0;
	foreach($counts as $v){
		$totalNum += $v['num'];
	}
} else if ($infos['ctype'] == 2) {
	$num = array();
	foreach($activitytype as $key=>$val) {
		$num[$key] = 0;
	}
	foreach($counts as $v){
		foreach($v['serverItems'] as $key1=>$val1) {
			$num[$key1] += $val1['num'];
		}
	}
}
...
include get_admin_tpl('servicetimeCount');
?>