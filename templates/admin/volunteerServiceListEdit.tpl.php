...
<form action='volunteerServiceList.php' method='post' class="searchFrom">
   			<table class="base_table">
    			<tbody>
    				<tr>
    					<th><span style="color: red;">*&nbsp;</span>栏目：</th><td colspan="4"><input readonly="readonly" type="text" name="name"  value="<?php echo $column['name'];?>" class="input length_5"/></td>
    				</tr>
    				<tr>
    					<th><span style="color: red;">*&nbsp;</span>标题：</th><td colspan="4"><input type="text" name="title" id="title"  maxlength="26" value="<?php echo $detail['title'];?>" class="input length_5"/></td>
    				</tr>
    				<tr>
    					<th>缩略图：</th><td colspan="3"><div class="thumb"><?php if($detail['pictures']){?> <img src="<?php echo $detail['pictures']?>" > <?php }else{ echo "生成160px*120px缩略图";} ?></div></td><td><?php if($now_flag){ ?><input type="button" id="uploadButton" value="上传图片"  /><?php } ?></td>
    				</tr>
    				<tr>
    					<th><span style="color: red;">*&nbsp;</span>内容：</th><td colspan="4"><textarea name="content" id="cont" class="cont" style="width:650px; height:400px;"  ><?php echo $detail['content'];?></textarea></td>
    				</tr>
    				
    				<tr>
    				
    				<input   type="hidden" id="thumb_url" name="thumb_url"  value="<?php if($detail['pictures']) {echo $detail['pictures'];}?>" />
    				<?php if($isshow==1){?>
    				<td ></td>
	    					<td><input type="hidden"  name="rid"  class="btn" value="<?php echo $rid;?>" /></td>
	    					<td><input type="hidden"  name="act"  class="btn" value="add" id="act"  /></td>
	    					<td><input type="submit"  name="save"  class="btn"  id="save" value="保存" />&nbsp;&nbsp;&nbsp;<a href="volunteerService.php" class="btn">返回</a></td>
    				<?php }else{?>
    				     <td></td>
    					<td><input type="hidden"  name="rid"  class="btn" value="<?php echo $detail['recordid'];?>" /></td>
    					<td><input type="hidden"  name="act"  class="btn" value="save" id="act"  /></td>
    					<td><?php if($now_flag){ ?><input type="submit"  name="save"  id="save" class="btn" value="保存" /><?php } ?>&nbsp;&nbsp;&nbsp;<a href="<?php echo $backurl;?>" class="btn">返回</a></td>
    				<?php }?>
    				</tr>
    	 		</tbody>
    			</table>
