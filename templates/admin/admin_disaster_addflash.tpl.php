...
<div id="content">
    <div class="con">
        ...
        <div class="admin_right">
            ...
            <div class="upflash">
                <form action="admindisasteraddflash.php" method="post" enctype="multipart/form-data" id="addflash">
                    <table>
                        <input type="hidden" name="rid" value="<?php echo $one['id']; ?>"/>
                        <tr>
                            <td><p><span class="must">*</span>&nbsp;&nbsp;排序号：</p></td>
                            <td><input type="text" name="order" value="<?php echo $one['orderlist'] ?>"
                                       class="input length_1"/>&nbsp;&nbsp;排序号最多为2位的正整数
                            </td>
                        </tr>
                        <tr>
                            <td><p><span class="must">*</span>&nbsp;&nbsp;名　称：</p></td>
                            <td><input type="text" name="name" value="<?php echo $one['name'] ?>"
                                       class="input length_3"/></td>
                        </tr>
                        <tr>
                            <td><p><span class="must">*</span>&nbsp;&nbsp;超链接：</p></td>
                            <td><input type="text" name="url" value="<?php echo $one['url'] ?>" class="input length_3"/>　例如：http://www.honghui.com
                            </td>
                        </tr>
                        <tr>
                            <td><p><span class="must">*</span>&nbsp;&nbsp;图　片：</p></td>
                            <td>
                                <div class="thumb"><?php if ($one['img']) { ?> <img
                                            src="../<?php echo $one['img'] ?>"> <?php } else {
                                        echo "建议上传1440px*415px图片(2M以内)";
                                    } ?></div>
                            </td>
                            <td><?php if ($now_flag) { ?><input type="button" id="uploadButton" value="上传图片"/><?php } ?>
                            </td>
                        </tr>
                        <input type="hidden" id="thumb_url" name="thumb_url" value="<?php if ($one['img']) {
                            echo "../" . $one['img'];
                        } ?>"/>
                    </table>
                    <div class="save">
                        <?php if ($now_flag) { ?><input type="submit" class="addbtn" name="dosave"
                                                        value="保存"/><?php } ?><a href="adminDisasterflash.php">
